...
- 10 라인, 설정한 경로는 request 내장 객체의 getContextPath()로 얻어온 컨텍스트 루트임. 이클립스에서는 프로젝트명이 컨텍스트 루트로 사용됨. "/MustHaveJSP_origin" 반환되어 설정됨. 웹 어플리케이션 전체에서 쿠키를 사용하겠다는 의미임.
...
```jsp
<!-- 레이어 팝업창 관리(ver 0.1). PopupMain_0.1.jsp -->
<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<%
String popupMode = "on";
...
- 5 라인, popupMode는 레이어 팝업창을 띄울지를 알려주는 변수. 이 값이 "on"이면 39 라인에서 팝업창을 보여주게 처리함.
- 24~30 라인, jQuery를 이용하는 자바스크립트 코드. id가 closeBtn인 요소(닫기 버튼)를 클릭하면 실행되는 함수로, hide()를 사용하여 팝업창을 숨김 처리함.
...
- 7~17 라인, 쿠키를 읽어 이름이 "PopupClose"인 쿠키가 존재하면 popupMode 변수의 값을 쿠키의 값으로 갱신함. 어딘가에서 PopupClose 쿠키의 값을 "on"이 아닌 값으로 설정했다면 팝업창이 더는 뜨지 않음.
- 39~48 라인, 팝업창을 숨김 처리하는 데서 끝나지 않고 [하루 동인 열지 않음]을 체크했는지를 확인하여 쿠키를 설정하는 페이지인 PopupCookie.jsp를 실행함.
- `var chkVal = $("input:checkbox[id=inactiveToday]:checked").val();`
  - id가 "inactiveToday"이면서 'checked 체크된' 체크박스의 값을 .val() 로 읽어와서 chkVal 변수에 저장한다는 뜻.
  - [하루 동안 열지 않음] 체크박스를 체크하면 chkVal 변수에 "1"이 저장되고, 체크하지 않으면 아무 값도 저장되지 않음.
...
```jsp
<!-- 쿠키를 생성해 응답 객체에 추가. PopupCookie.jsp -->
<%@ page language="java" contentType="text/html; charset=UTF-8"
...
- 5 라인, 체크박스 폼값을 받음. [오늘 하루 열지 않음] 체크박스를 체크했다면 이 값으로 "1"이 전달됨.
- 7~13 라인, 값이 "1"이면 이름이 "PopupClose", 값이 "off", 경로는 컨텍스트 루트, 유지 기간은 하루인 쿠키를 생성해 응답 객체에 추가함.
- 12 라인, 출력하는 문자열은 "PopupMain.jsp"의 ajx() success의 요청 성공시 호출되는 함수로 콜백됨.
- 시간 흐름 순으로 정리
  - 폼 체크하고 닫기 버튼 클릭
  - 팝업창 hide됨.
  - chkVal 변수에 id가 inactiveToday이고 checked된 체크박스의 value인 "1"을 저장함.
  - ajax() 함수 실행함.
  - 이름 inactiveToday 값 "1" 쌍을 GET 방식으로 PopupCookie.jsp로 보냄
  - PopupCookie.jsp에서 request.getParameter로 inactiveToday 값을 읽어서 chkVal 변수에 저장함.
  - chkVal 값이 null이 아니고 "1"이면 이름이 PopupClose이고 값이 "off"인 쿠키 생성함.
  - response.addCookie(cookie) 로 HTTP 응답 헤더에 실어 클라이언트로 쿠키를 전송함.
  - out.println("쿠키 : 하루 동안 열지 않음") ???????????????모름??????????
  - 클라이언트 쿠키 받아 request 객체에 저장함.
  - ajax() 함수 success 되어 페이지 reload 실행됨.
  - request 객체의 전체 쿠키값을 읽어서 PopupClose 쿠키 있으면 PopupMode에 쿠키값인 "off" 저장.
  - 페이지에 "현재 팝업창은 off 상태입니다." 리로드됨. 팝업은 hide되어 보이지 않음.
  - 새로고침 누르면 PopupMode값 "on" 아니라서 팝업 실행 안됨.
...
- 7 라인, JspWriter 클래스를 임포트함. 기본 내장 객체인 out이 JspWriter 타입임.
- 11~21 라인, alertLocation()은 알림창으로 메시지를 보여준 후 명시한 URL로 이동하는 메소드임. 매개변수 3개를 받는데, 각각의 의미는 아래와 같음.
  - msg : 알림창에 띄울 메시지.
  - url : 알림창을 닫은 후 이동할 페이지의 URL.
  - out : 자바스크립트 코드를 삽입할 출력 스트림(JSP의 out 내장 객체). JSP의 내장 객체를 클래스에서 사용하려면 이와 같이 매개변수로 받아야 함.
- alertLocation() 메소드는 13~17 라인에서 자바스크립트 코드를 문자열 형태로 선언한 뒤, 18라인에서 out 객체로 출력함.
- 24~34 라인, alertBack() 메소드는 알림창을 띄운 후 다른 페이지로 이동하는데, 호출자가 URL을 지정할 수 없고, 무조건 이전 페이지로 이동함. history.back(); 활용.
...
- 18~19 라인, 표현식을 사용해 [아이디] 입력창과 [아이디 저장하기] 체크박스의 기본값을 설정함. 표현식에서 사용하는 loginId와 cookieCheck 변수의 값은 코드 상단의 JSP 코드에서 결정됨.
- 6 라인, 앞에서 작성한 CookieManager 클래스를 이용하여 이름이 "loginId"인 쿠키를 읽어와 loginId 변수에 저장함. 저장되어 있는 아이디가 있다면 이 값이 18 라인 [아이디] 입력창의 기본값으로 쓰이게 됨.
- 8~11 라인, cookieCheck 변수의 값을 결정함. 쿠키에 저장된 아이디가 있다면(loginId에 빈 문자열 외의 문자열을 저장해뒀다면) cookieCheck에 "checked"가 저장됨. 따라서 19 라인에서 [아이디 저장하기] 체크박스에 checked 속성을 부여함.
...
```jsp
<!-- 로그인 및 아이디 저장. IdSaveProcess.jsp -->
<%@ page import="utils.CookieManager"%>
...
- 7~9 라인, request 내장 객체를 통해 전송된 폼값을 받아서 변수에 저장함.
- 11 라인, user_id와 user_pw를 하드코딩된 값과 비교함.
- 13 라인, 로그인 성공 시 [아이디 저장하기] 체크 여부를 확인함.
- 14 라인, 체크되어 있다면 쿠키를 생성함.
- 16~18 라인, 체크되어 있지 않다면 기존 쿠키를 삭제함.
- 20 라인, 로그인 성공 알림창을 띄워준 후 메인 페이지(IdSaveMain.jsp)로 이동함.
- 24 라인, 로그인에 실패하면 단순히 로그인 실패 알림창을 띄워준 후 이전 페이지로 돌아감.
