...
public class MyClass {
	// 필드
	RemoteControl rc = new Television(); // 인터페이스 필드로 선언
	
	// 생성자
	MyClass(){
	}
	
	MyClass(RemoteControl rc){ // 인터페이스 생성자 매개변수로 선언
		this.rc = rc;
		rc.turnOn();
		rc.setVolume(2);
	}
	...
	// 메소드
	void methodA() {
		RemoteControl rc = new Audio(); // 인터페이스 메소드 로컬변수로 선언
		rc.turnOn();
		rc.setVolume(3);
	}
	
	void methodB(RemoteControl rc) { // 인터페이스 메소드 매개변수로 선언
		rc.turnOn();
		rc.setVolume(4);
	}
}
...
- 1. 자동차의 필드 타입으로 타이어 인터페이스를 선언하면 필드값으로 구현 객체인 한국 타이어 또는 금호 타이어 객체를 대입할 수 있음. 자동 타입 변환이 일어나서 문제가 없음.
...
- 예를 들어 인터페이스에 3개의 메소드가 선언되어 있고 구현 클래스에 재정의된 메소드 3개 포함 총 5개의 메소드가 선언되어 있다면, 자동 타입 변환된 인터페이스로 호출 가능한 메소드는 3개뿐임.
