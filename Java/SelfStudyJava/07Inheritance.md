...
- <u>상속의 자동 타입 변환은 일종의 접근 제한임. 더 넓은 범위의 자식 클래스에서 더 좁은 범위의  부모 클래스로 멤버 접근이 제한된 것.</u>
...
- 매개 변수의 타입이 클래스일 경우, 해당 클래스의 객체뿐만 아니라 자식 객체까지고 매개값으로 사용할 수 있음.
...
- 예외가 발생하면 프로그램은 즉시 종료되기 때문에 강제 타입 변환을 하기 전에 instanceof 연산자로 변환시킬 타입의 객체인지 조해서 잘못된 매개값으로 인해 프로그램이 종료되는 것을 막아야 함.
