...
- 레턴값 : 메소드가 실행되고 난 후 호출한 곳으로 돌려주는(리턴하는) 값.
...
- 클래스에 생성자가 명시적으로 선언되어 있을 경우, 반드시 선언된 생성자를 호출해서 객체를 생성해야 하고 이 과정에서 매개값도 제공되어야 함. 클래스에 생성자 선언이 있다면 기본 생성자(Car())를 호출해서 객체를 생성할 수 없음. 매가변수가 있는 생성자를 호출해야 함.
