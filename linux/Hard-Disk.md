...
1. VMware 가상머신 생성. 하드디스크 3개 추가 (총 4개)
2. 페도라 ISO 파일 넣고 설치 진행.
3. 초기 화면에서 Install to Hard Dirve 클릭.
4. 언어 설정 한국어, INSTALLATION DESTINATION 클릭.
5. 설치 목적지 화면에서 추가하려는 하드디스크 한 번씩 클릭해서 V체크 확인한 후 Custom 클릭, Done 클릭.
6. 수동으로 파티션 설정 화면에서 표준 파티션으로 변경 → 아래쪽의 + 클릭 → 새 마운트 지점 추가에서 마운트 지점은 swap 선택 → 원하는 용량 4G → Add mount point
7. 왼쪽의 swap 선택하고 오른쪽의 Device Type을 RAID, RAID Level을 RAID5로 설정.

![Untitled](C:\Users\MIT-김재준\Downloads\Export-1ad2ec76-76df-4f2b-bf65-fcbbfad69620\하드디스크 관리 16ee87d214204310b5fff9682476c746\Untitled 5.png)

1. 다시 왼쪽 아래 + 클릭해서 동일한 방식으로 / 파티션을 RAID5로 구성.
2. 왼쪽 위 Done 클릭하고 변경 요약의 오른쪽 아래 Accept Changes 클릭.

결함 발생 시 원상 복구

1. 하드디스크 1개 추가
2. `fdisk -l /dev/sda` 명령 입력해 /dev/sda의 파티션을 확인하고, 동일하게 /dev/sdb 장치의 파티션을 설정함.
3. `mdadm /dev/md/swap —add /dev/md/sdb1` , `mdadm /dev/md/root —add /dev/md/sdb2` 명령 입력.